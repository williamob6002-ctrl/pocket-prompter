...
| Media export | Implemented: separate Photos/share and file-download actions, trimmed WAV audio, SRT | User confirmed recording/file download on a phone. Revised Photos sharing and school-submission destinations still require target-device confirmation. |
...
| AI script and eye-contact tools | Not implemented | A small local writing model failed three supplied-fact school-script quality checks and was excluded. Larger writing models and eye-contact tools need separate device/quality validation. These are missing features, not a blanket claim of web impossibility. No paid service is silently substituted. |
