...
import {processVideo, toSrt, parseSrt, getVideoProcessingSupport} from './video-processing.js';
import {VoiceFollower} from './voice-follow.js';

const $ = selector => document.querySelector(selector);
const $$ = selector => [...document.querySelectorAll(selector)];
const STORE = 'pocket-prompter-v1';
const defaults = {wpm:130,fontSize:40,lineHeight:1.6,width:90,font:'system',align:'left',theme:'dark',guide:true,mirrorX:false,mirrorY:false,countdown:3,loop:false,keepAwake:true,voice:false,language:'en-GB',facing:'user',quality:'1080',panelHeight:60,opacity:70};
...
let exportController=null,previewTrimEnd=null;
...
function normaliseSettings(){
  const ranges={wpm:[40,300],fontSize:[24,88],lineHeight:[1.35,2.2],width:[45,100],countdown:[0,10],panelHeight:[45,80],opacity:[20,95]};
...
function applySettings(){
  ...
  else if(cameraMode)$('#voice-help').textContent='Voice following is available in practice mode. Recording uses fixed-speed scrolling so both features do not compete for the microphone.';
  ...
  for(const key of ['facing','quality'])$(`[data-setting="${key}"]`).disabled=recordingStarting||!!recorder&&recorder.state!=='inactive';
}
for(const input of $$('[data-setting]'))input.oninput=async()=>{
  ...
  if(cameraMode&&['facing','quality'].includes(key)&&(!recorder||recorder.state==='inactive'))await startCamera();
};
...
async function openReader(camera){
  save();cameraMode=camera;readerOpen=true;sessionElapsed=0;$('#workspace').hidden=true;$('#reader').hidden=false;$('#reader').classList.toggle('camera-mode',camera);document.body.classList.add('reading');$('#record-toggle').hidden=!camera;$('#finish-recording').hidden=true;$('#camera-message').hidden=true;$('#session-status').textContent=camera?'Opening camera…':'Ready';$('#elapsed').textContent='0:00';$('#record-toggle').disabled=camera;renderPrompt();applySettings();setProgress(0);updateButtons();
  if(camera)await startCamera();else if(settings.keepAwake)await keepAwake();
}
function stopCamera(){cameraRequest++;if(stream){for(const track of stream.getTracks())track.stop();stream=null;}$('#camera-preview').srcObject=null;}
async function startCamera(){
  stopCamera();const request=cameraRequest;$('#record-toggle').disabled=true;$('#camera-message').hidden=true;
  ...
  try{
    const shortSide=Number(settings.quality),portrait=window.innerHeight>window.innerWidth;const newStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:settings.facing},width:{ideal:portrait?shortSide:Math.round(shortSide*16/9)},height:{ideal:portrait?Math.round(shortSide*16/9):shortSide},aspectRatio:{ideal:portrait?9/16:16/9},frameRate:{ideal:30,max:30}},audio:{echoCancellation:true,noiseSuppression:true}});
    if(!readerOpen||!cameraMode||request!==cameraRequest){newStream.getTracks().forEach(t=>t.stop());return;}
    stream=newStream;$('#camera-preview').srcObject=stream;await $('#camera-preview').play();$('#record-toggle').disabled=false;const actual=stream.getVideoTracks()[0]?.getSettings();$('#session-status').textContent=`Camera ready${actual?.height?` · ${actual.width}×${actual.height}`:''}`;
  ...
  }catch(e){if(request!==cameraRequest)return;const denied=['NotAllowedError','PermissionDeniedError'].includes(e.name);$('#camera-message').textContent=denied?'Camera or microphone access was denied. Allow both in Safari’s website settings, then close and reopen the recording screen. Your script is safe.':'The camera could not start. Close other camera apps, check that a microphone is available, then try again.';$('#camera-message').hidden=false;$('#session-status').textContent=denied?'Permission needed':'Camera unavailable';}
}
...
async function play(){
  if(pendingStart){pause();return;}if(playing){pause();return;}if(progress()>=.999)setProgress(0);
...
function tick(now){
  if(readerOpen){const delta=lastFrame?Math.min((now-lastFrame)/1000,.1):0;lastFrame=now;
    if(playing){sessionElapsed+=delta;if(!voiceFollower){const seconds=wordCount(current.text)/settings.wpm*60;scrollPosition+=maxScroll()/Math.max(seconds,1)*delta;$('#prompt-scroll').scrollTop=scrollPosition;updatePosition();if(scrollPosition>=maxScroll()){if(settings.loop&&!cameraMode)setProgress(0);else{playing=false;$('#session-status').textContent=recorder?.state==='recording'?'Script finished · tap Finish':'Finished';updateButtons();}}}}
...
$('#record-toggle').onclick=async()=>{
  ...
  recordingStarting=true;pause();updateButtons();
  let preparedTake;
  try{
    ...
    const take={id:uid(),title:current.title||'Untitled script',created:Date.now(),mimeType:activeRecorder.mimeType||mime,duration:0,bytes:0,width:videoSettings.width,height:videoSettings.height,status:'recording',script:plainText(current.text)};
    preparedTake=take;
...
function trimRange(){const start=Number($('#trim-start').value),end=Number($('#trim-end').value);if(!Number.isFinite(start)||!Number.isFinite(end)||start<0||end<=start)throw new Error('End time must be later than the start time.');return {start,end};}
...
$('#export-edit').onclick=async()=>{
  ...
  try{
    ...
    const logo=$('#video-logo').files[0],music=$('#video-music').files[0];if(logo?.size>10*1024*1024||music?.size>50*1024*1024)throw new Error('Choose a picture under 10 MB and music under 50 MB.');
    $('#take-player').pause();previewTrimEnd=null;exportController=new AbortController();$('#export-progress').hidden=false;$('#export-edit').disabled=true;$('#take-dialog .dialog-heading button').disabled=true;$('#delete-take').disabled=true;
    const result=await processVideo({blob:selectedBlob,...range,captions,aspect:$('#video-aspect').value,logo,music,musicVolume:Number($('#music-volume').value)/100,signal:exportController.signal,onProgress:info=>{$('#export-progress progress').value=info.progress;$('#export-progress p').textContent=info.phase==='rendering'?`Creating your copy… ${Math.round(info.progress*100)}%`:'Preparing video…';}});
    const edited={...take,id:uid(),title:`${take.title} (edited)`,created:Date.now(),duration:result.duration,mimeType:result.mimeType,width:result.width,height:result.height,bytes:result.blob.size,status:'saved',script:undefined};
  ...
  finally{exportController=null;$('#export-edit').disabled=false;$('#take-dialog .dialog-heading button').disabled=false;$('#delete-take').disabled=false;$('#export-progress').hidden=true;}
};
